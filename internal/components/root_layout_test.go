...
func TestCalculateRootLayoutSplitsExactly(t *testing.T) {
	layout := calculateRootLayout(200, 60, true)
...
func TestCalculateRootLayoutNeverNegative(t *testing.T) {
	for _, hasTabs := range []bool{true, false} {
		for w := 1; w <= 200; w++ {
			for h := 1; h <= 80; h++ {
				layout := calculateRootLayout(w, h, hasTabs)
				...
				if layout.leftWidth < 0 || layout.rightWidth < 0 || layout.tabBarHeight < 0 ||
					...
					t.Fatalf("negative layout values for tabs=%v w=%d h=%d: %+v", hasTabs, w, h, layout)
				}

				if layout.leftWidth+layout.rightWidth != w {
					t.Fatalf("width split mismatch for tabs=%v w=%d h=%d: %+v", hasTabs, w, h, layout)
				}

				if layout.tabBarHeight+layout.tableHeight+layout.bottomHeight != layout.contentHeight {
					t.Fatalf("content-height split mismatch for tabs=%v w=%d h=%d: %+v", hasTabs, w, h, layout)
				}

				if layout.sqlWidth+layout.logWidth != layout.rightWidth {
					t.Fatalf("sql/log split mismatch for tabs=%v w=%d h=%d: %+v", hasTabs, w, h, layout)
				}
			}
		}
	}
